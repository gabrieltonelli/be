...
class CustomPopupMenu {
  ...
  String title;
  IconData icon;
  CustomPopupMenu({this.itemId, this.title, this.icon});
}
...
Widget Be_PopupContactMenu(){
  ...
  List<CustomPopupMenu> choices = <CustomPopupMenu>[
    CustomPopupMenu(itemId: "omitir", title: "Omitir contacto", icon: Icons.visibility_off),
  ];
    ...
    itemBuilder: (BuildContext context){
      return choices.map((CustomPopupMenu choice){
        return PopupMenuItem<String>(
          value: choice.itemId,
          child: Row(children: <Widget>[Padding(padding: EdgeInsets.only(right: 7), child:Icon(choice.icon,color: Colors.grey,)),Text(choice.title)],),
...
Widget Be_PopupAttributeMenu(){
  ...
  List<CustomPopupMenu> choices = <CustomPopupMenu>[
    CustomPopupMenu(itemId: "omitir", title: "Omitir tarjeta", icon: Icons.visibility_off),
  ];
